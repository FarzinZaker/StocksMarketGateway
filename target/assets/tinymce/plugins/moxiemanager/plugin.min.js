	...
	function loadAndRender(view) {
		return function(settings) {
			...
			function done() {
				...
				moxman.util.JsonRpc.url = getMoxieManagerApiPage();
			...
			if (!moxman.ui) {
				ResourceLoader.load({
					js: [
                        ...
                        getMoxieManagerApiPage() + "?act=language" + (settings.language ? '&code=' + settings.language : ''),
                        getMoxieManagerApiPage() + "?act=PluginJs"
					],
...
define("moxman/interop/TinyMcePlugin", [
	...
	if (langCode && langCode != 'en') {
		tinymce.ScriptLoader.load(getMoxieManagerApiPage() + '?act=language&tinymce=true&code=' + langCode);
	}
});
