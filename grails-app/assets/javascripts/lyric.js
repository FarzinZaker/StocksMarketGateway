...
function ordinalize(dataSeries) {
    // Build the new array
    var vectorArray = [];
    vectorArray['x'] = [];		// this will be the ordinal
    vectorArray['y'] = [];		// this will be the 'y' value of the input
    vectorArray['label'] = []; // this will be the 'x' value of the input
...
function applyRegression(data, degree, trendCount) {
    ...
    for (var j = 0; j < data.length; j++) {
        input['x'][j] = j;
        input['y'][j] = data.dataset[serieIndex].data[j].value.toString();
    }
    var result = buildModel(input, degree);
    var estimationInput = [];
    estimationInput['x'] = [];
    for (j = 0; j < trendCount; j++) {
        estimationInput['x'][j] = data.dataset[serieIndex].data.length - trendCount + j;
    }
    var estimateData = applyModel(estimationInput, result, degree);
    for (j = 0; j < trendCount; j++) {
        data.dataset[serieIndex].data[data.dataset[serieIndex].data.length - trendCount + j].value = estimateData[j].y
    }
    return data;
}