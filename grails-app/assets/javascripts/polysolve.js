...
function applyRegression(data, degree, trendCount) {
    for (var i = 0; i < data.dataset.length; i++) {
        var pairs = []
        for (var j = 1; j < data.dataset[i].data.length - trendCount; j++) {
            pairs[pairs.length] = new Pair(j, data.dataset[i].data[j].value);
        }
        var result = matf.process_data(pairs, degree)[0];
        for(var j = data.dataset[i].data.length - trendCount; j < data.dataset[i].data.length; j++)
            data.dataset[i].data[j].value = matf.regress(j, result);
    }
