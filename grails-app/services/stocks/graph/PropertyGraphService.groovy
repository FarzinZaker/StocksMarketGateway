...
class PropertyGraphService {
    ...
    List<Map> propertyCloud() {
        graphDBService.queryAndUnwrapVertex("SELECT @rid, @class as label, identifier, title, IN('About').size() AS count FROM Property GROUP BY @rid ORDER BY count DESC LIMIT 50")
    }
    ...
    List<Map> propertyCloud(String propertyId) {
        graphDBService.queryAndUnwrapVertex("SELECT * FROM (SELECT EXPAND(IN('About').OUT('About')) FROM Property WHERE @rid = #${propertyId}) WHERE @rid <> #${propertyId} LIMIT 50")
    }
