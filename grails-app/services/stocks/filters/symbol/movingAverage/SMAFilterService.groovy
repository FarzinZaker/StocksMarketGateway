    ...
    Boolean check(Symbol symbol, String parameter, String operator, Object value, Date date) {
        ...
        switch (operator) {
            case Operators.UPPER_THAN:
                if (targetIndicator)
                    return indicatorCompareService.indicatorUpperThanIndicator(symbol, SMA, parameter, targetIndicator, targetParameter, date)
                else
                    return indicatorCompareService.indicatorUpperThanPrice(symbol, SMA, parameter, date)
            case Operators.LOWER_THAN:
                if (targetIndicator)
                    return indicatorCompareService.indicatorLowerThanIndicator(symbol, SMA, parameter, targetIndicator, targetParameter, date)
                else
                    return indicatorCompareService.indicatorLowerThanPrice(symbol, SMA, parameter, date)
            case Operators.CROSSING_TO_UP:
                if (targetIndicator)
                    return indicatorCompareService.indicatorCrossUpIndicator(symbol, SMA, parameter, targetIndicator, targetParameter, date)
                else
                    return indicatorCompareService.indicatorCrossUpPrice(symbol, SMA, parameter, date)
            case Operators.CROSSING_TO_DOWN:
                if (targetIndicator)
                    return indicatorCompareService.indicatorCrossDownIndicator(symbol, SMA, parameter, targetIndicator, targetParameter, date)
                else
                    return indicatorCompareService.indicatorCrossDownPrice(symbol, SMA, parameter, date)
        }
        false
    }
