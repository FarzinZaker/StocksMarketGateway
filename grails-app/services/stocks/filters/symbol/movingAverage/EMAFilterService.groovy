    ...
    Boolean check(Symbol symbol, String parameter, String operator, Object value, Date date, String adjustmentType) {
        ...
        switch (operator) {
            case Operators.UPPER_THAN:
                if (targetIndicator)
                    return indicatorCompareService.indicatorUpperThanIndicator(symbol, EMA, parameter, targetIndicator, targetParameter, date, adjustmentType)
                else
                    return indicatorCompareService.indicatorUpperThanPrice(symbol, EMA, parameter, date, adjustmentType)
            case Operators.LOWER_THAN:
                if (targetIndicator)
                    return indicatorCompareService.indicatorLowerThanIndicator(symbol, EMA, parameter, targetIndicator, targetParameter, date, adjustmentType)
                else
                    return indicatorCompareService.indicatorLowerThanPrice(symbol, EMA, parameter, date, adjustmentType)
            case Operators.CROSSING_TO_UP:
                if (targetIndicator)
                    return indicatorCompareService.indicatorCrossUpIndicator(symbol, EMA, parameter, targetIndicator, targetParameter, date, adjustmentType)
                else
                    return indicatorCompareService.indicatorCrossUpPrice(symbol, EMA, parameter, date, adjustmentType)
            case Operators.CROSSING_TO_DOWN:
                if (targetIndicator)
                    return indicatorCompareService.indicatorCrossDownIndicator(symbol, EMA, parameter, targetIndicator, targetParameter, date, adjustmentType)
                else
                    return indicatorCompareService.indicatorCrossDownPrice(symbol, EMA, parameter, date, adjustmentType)
        }
        false
    }
