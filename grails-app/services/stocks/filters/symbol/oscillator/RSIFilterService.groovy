    ...
    Boolean check(Symbol symbol, String parameter, String operator, Object value, Date date) {
        ...
        switch (operator) {
            case Operators.UPPER_THAN:
                return indicatorCompareService.indicatorUpperThanValue(symbol, RSI, parameter, targetValue, date)
            case Operators.LOWER_THAN:
                return indicatorCompareService.indicatorLowerThanValue(symbol, RSI, parameter, targetValue, date)
            case Operators.CROSSING_TO_UP:
                return indicatorCompareService.indicatorCrossUpValue(symbol, RSI, parameter, targetValue, date)
            case Operators.CROSSING_TO_DOWN:
                return indicatorCompareService.indicatorCrossDownValue(symbol, RSI, parameter, targetValue, date)
        }
        false
    }
