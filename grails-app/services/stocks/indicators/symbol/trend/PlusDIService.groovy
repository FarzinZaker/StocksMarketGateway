...
class PlusDIService implements IndicatorServiceBase<Symbol, Integer> {
    ...
    Double calculate(Symbol item, Integer parameter, Date date) {
        ...
        core.plusDI(0, parameter - 1, TypeCast.toDoubleArray(highSeries), TypeCast.toDoubleArray(lowSeries), TypeCast.toDoubleArray(closeSeries), parameter, beginIndex, endIndex, result)
        result?.toList()?.first()
    }
}
