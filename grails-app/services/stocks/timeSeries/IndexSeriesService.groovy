...
class IndexSeriesService {
    ...
    def valueList(Long indexId, String property, Date startDate = null, Date endDate = null, String groupingMode = '1d') {
        ...
        if (!endDate)
            endDate = new Date()
        def values = timeSeriesDBService.query("SELECT LAST(value) FROM index_${indexId}_${property} WHERE time >= ${startDate.time * 1000}u and time <= ${endDate.time * 1000}u GROUP BY time(${groupingMode})")[0]?.series?.values
    ...
    Double lastValue(Long indexId, String property, Date endDate = null){
        if (!endDate)
            endDate = new Date()
        def values = timeSeriesDBService.query("SELECT LAST(value) FROM index_${indexId}_${property} WHERE time <= ${endDate.time * 1000}u")[0]?.series?.values
