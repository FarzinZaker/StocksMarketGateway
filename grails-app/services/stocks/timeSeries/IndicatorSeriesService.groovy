...
class IndicatorSeriesService {
    ...
    def indicatorList(Long symbolId, Class<IndicatorBase> indicator, String parameter, Date startDate = null, Date endDate = null, String groupingMode = '1d', String adjustmentType = null) {
        ...
        def values = timeSeriesDBService.query("SELECT LAST(value) FROM indicator_${indicator.canonicalName.replace('.', '_').replace('stocks_indicators_symbol_', '')}_${parameter?.replace(',', '_')}${adjustmentType}_${symbolId} WHERE time >= ${startDate.time * 1000}u and time <= ${endDate.time * 1000}u GROUP BY time(${groupingMode})")[0]?.series?.values
    ...
    Double lastIndicator(Long symbolId, Class<IndicatorBase> indicator, String parameter, Date endDate = null, String adjustmentType = null) {
        ...
        if (!adjustmentType)
            adjustmentType = AdjustmentHelper.defaultType
        def values = timeSeriesDBService.query("SELECT LAST(value) FROM indicator_${indicator.canonicalName.replace('.', '_').replace('stocks_indicators_symbol_', '')}_${parameter?.replace(',', '_')}${adjustmentType}_${symbolId} WHERE time <= ${endDate.time * 1000}u")[0]?.series?.values
