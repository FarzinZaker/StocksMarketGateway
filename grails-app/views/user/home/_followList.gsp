        ...
        <g:each in="${followList}" var="follow" status="indexer">
            <li class="${indexer % 2 ? 'even' : 'odd'}">
                <div class="image"><img width="60px"
                                        src="${createLink(controller: 'image', action: 'profile', id: follow.identifier)}"/>
                </div>

                <div class="description">
                    <h4>${follow.title}</h4>

                    <a class="k-button" href="${createLink(controller: follow.label == 'Person' ? 'user' : 'twitter', action: follow.label == 'Person' ? 'wall' : 'property', id: follow.identifier)}"><g:message
