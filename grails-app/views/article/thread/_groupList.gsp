<g:each in="${groupList}" var="${group}">
    <div class="group" item-id="${group.idNumber}" >
        ...
        <div class='toolbar'>
            <a href='${createLink(controller: 'group', action: 'home', id: group.id)}'><g:message code="twitter.group.view.button"/></a><span onclick='registerInGroup("${group.idNumber}", "${group.title}")'><g:message code="twitter.group.register.button"/></span>
        </div>
    </div>
</g:each>